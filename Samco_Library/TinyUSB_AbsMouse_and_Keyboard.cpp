 ...
 */
  ...
  uint8_t const desc_hid_report[] = {
    ...
    0x85, RID_MOUSE,    //     Report ID (REPORT_ID_MOUSE)
    /* 3 bits Buttons (Left, Right, Middle) */
    ...
    0xa1, 0x01,         // COLLECTION (Application)
    0x85, RID_KEYBOARD, //   REPORT_ID (REPORT_ID_KEYBOARD)
  ...
  TinyAbsMouse_::TinyAbsMouse_(void) {
    _buttons = 0;
    _x = 0;
    _y = 0;
    _width = 32767;
  ...
  void TinyAbsMouse_::end(void) {
  }

  void TinyAbsMouse_::init(uint16_t width, uint16_t height, bool autoReport) {
    _width = width;
    _height = height;
    _autoReport = autoReport;
  }

  void TinyAbsMouse_::report(void) {
    uint8_t buffer[5];
    buffer[0] = _buttons;
    buffer[1] = _x & 0xFF;
    buffer[2] = (_x >> 8) & 0xFF;
    ...
    usb_hid.sendReport(RID_MOUSE, buffer, 5);
  }

  void TinyAbsMouse_::move(uint16_t x, uint16_t y) {
  ...
  void TinyAbsMouse_::press(uint8_t button, bool force) {
    _buttons |= button;

    if (_autoReport || force) {
      report();
    }
  }

  void TinyAbsMouse_::release(uint8_t button, bool force) {
    _buttons &= ~button;
  ...
  void TinyAbsMouse_::click(uint8_t button) {
    press(button, true);
    release(button, true);
  }

  bool TinyAbsMouse_::isPressed(uint8_t button) {
    if (button & _buttons) {
      return true;
    }
    return false;
  ...
  TinyAbsMouse_ AbsMouse; // create an instance of the Mouse object
  ...
  void TinyKeyboard_::init(bool autoReport) {
    _autoReport = autoReport;
  }

  void TinyKeyboard_::report(void) {
    if (USBDevice.suspended())  {
      USBDevice.remoteWakeup();
    }
    if(!usb_hid.ready()) return;
    usb_hid.sendReport(RID_KEYBOARD, &_keyReport, sizeof(KeyReport));
  }
  ...
  size_t TinyKeyboard_::press(uint8_t k, bool force) {
    uint8_t i;
    if (k >= 136) {     // it's a non-printing key (not a modifier)
      ...
      if (k & 0x80) { // it's a capital letter or other character reached with shift
        ...
        _count++; // keep track of how many keys are using the left shift modifier
        k &= 0x7F;
      }
    }
  ...
  size_t TinyKeyboard_::release(uint8_t k, bool force) {
    uint8_t i;
    if (k >= 136) {     // it's a non-printing key (not a modifier)
      k = k - 136;
    } else if (k >= 128) {  // it's a modifier key
      ...
      if (k & 0x80) { // it's a capital letter or other character reached with shift
        _count--;
        if (!_count) { // only unset the left shift modifier if this is the last key using it
          _keyReport.modifiers &= ~(0x02); // the left shift modifier
        }
        k &= 0x7F;
      }
  ...
  // Untested, will sending two reports without yelding to tud_task work?
  size_t TinyKeyboard_::write(uint8_t c) {
    uint8_t p = press(c, true); // Keydown
    release(c, true);           // Keyup
